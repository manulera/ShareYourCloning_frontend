...
function ValidatedTextField({ inputRef, validator, updateValidationStatus, required, submissionAttempted, ...rest }) {
  ...
  React.useEffect(() => {
    ...
    if (required && value.length === 0) {
      ...
      updateValidationStatus(inputRef.current.id, false);
      return;
    }
    const { error: newError, helperText: newHelperText } = validator(value);
    setError(newError);
