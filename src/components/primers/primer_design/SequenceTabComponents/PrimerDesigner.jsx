...
function PrimerDesigner() {
  ...
  const { finalSource, otherInputIds, pcrSources } = useSelector((state) => getPrimerDesignObject(state.cloning), isEqual);
  ...
  // Nothing to design
  if (templateSequencesIds.length === 0 || pcrSources.length === 0) {
    return null;
  }
