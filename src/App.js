...
function App() {
  ...
  const [nextUniqueId, setNextUniqueId] = React.useState([1]);
  ...
  const [sources, setSources] = React.useState([{
    id: uniqueIdDispatcher(), input: [], output: null, type: null, kind: 'source',
  }]);
  ...
  const addSource = (inputEntity) => setSources(sources.concat([
    {
      id: uniqueIdDispatcher(), input: [inputEntity.id], output: null, type: null, kind: 'source',
    },
  ]));
